...
Write-Host ""
Write-Host "어떤 방식으로 배포하시겠습니까?" -ForegroundColor Cyan
Write-Host "1) Docker Hub에서 다운로드 받아 실행 (빠름, 권장)"
Write-Host "2) 이 PC에서 직접 소스 코드를 빌드하여 실행 (오래 걸림)"
...
switch ($choice) {
    ...
    "2" {
        Write-Host "🔨 소스코드를 직접 빌드하여 배포합니다..." -ForegroundColor Cyan
        docker compose up -d --build
    }
    default {
        Write-Host "❌ 잘못된 입력입니다. 스크립트를 종료합니다." -ForegroundColor Red
        Pause
        exit
    }
}
